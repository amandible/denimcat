...
import type { PlayerCount } from '@denimcat/engine-love-triangles';
import { createSocket } from '../../socket';
import '../../styles/love-triangles.css';

function goToRoom(code: string) {
  window.location.href = `/love-triangles/room/${code.toUpperCase()}`;
}

export function LoveTrianglesHomeView() {
  ...
  function handleCreate() {
    setBusy(true);
    const socket = createSocket('/love-triangles');
    socket.on('connect', () => {
      socket.emit(
        'create_room',
        { playerCount },
  ...
  return (
    <main style={{ maxWidth: 420, margin: '0 auto', minHeight: '100vh', display: 'flex', alignItems: 'center', padding: '0 16px' }}>
      <div className="panel" style={{ width: '100%' }}>
        <h1 style={{ fontSize: 28 }}>Love Triangles</h1>
        <p style={{ color: 'var(--muted)', marginTop: 0 }}>TODO: Love Triangles description</p>
